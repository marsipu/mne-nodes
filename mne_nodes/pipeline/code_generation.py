...
class CodeGenerator:
    ...
    def generate_code(self):
        ...
        code = self._build_header(import_function_names)
        # Iterate nodes
        for target, nodes in targets.items():
            ...
            for selection_type in [
                ...
                for n in nodes:
                    ...
                    for ip in inputs:
                        ...
                        if ip == "raw":
                            ...
                            if target == "file":
                                ...
                                code += self._indent(
                                    "raw, event_id = read_raw_bids(bp_raw, extra_params={'preload': True}, return_event_dict=True)\n",
                                    1,
                                )
                            else:
                                code += self._indent(
                                    "raw = (read_raw_bids(rp, extra_params={'preload': True}, return_event_dict=True)[0] for rp in members)\n",
                                    1,
                                )
                            ...
                            code += self._indent(
                                "subject = ct.load_subject(bp.subject)\n", 1
                            )
                    ...
                    func_inputs = self.ct.func_inputs(name, loaded_data)
                    if func_meta["class_name"] is not None:
                        lower_class = func_meta["class_name"].lower()
                        func_line += f"{lower_class}."
                        func_name = name.split(".")[-1]
                        if lower_class in func_inputs:
                            func_inputs.remove(lower_class)
                    else:
                        func_name = name
                    func_line += f"{func_name}("
                    if len(func_inputs) > 0:
                        inputs = ", ".join([f"{ip}={ip}" for ip in func_inputs])
