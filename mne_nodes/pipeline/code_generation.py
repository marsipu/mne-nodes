...
class CodeGenerator:
    ...
    def _build_header(self, function_names):
        code = (
            ...
            "# Load plugin namespaces\n"
        )
        plugin_names = sorted(
            {
                self.ct.get_plugin_from_function(function_name)
                for function_name in function_names
            }
        )
        for plugin_name in plugin_names:
            code += (
                f"{self._plugin_namespace(plugin_name)} = ct.plugins[{plugin_name!r}]\n"
            )
    ...
    def _plugin_namespace(plugin_name):
        """Return the generated-script namespace for a plugin."""
        namespace = re.sub(r"\W", "_", plugin_name)
        if namespace and namespace[0].isdigit():
            namespace = f"plugin_{namespace}"
        return namespace

    def _qualified_call(self, func_name, function_names):
        """Return a plugin-namespace reference for a registered function.

        Records non-method functions so their plugin namespace is available in
        the generated script header.
        """
        func_meta = self.ct.get_function_meta(func_name)
        if func_meta.get("class_name") is None:
            function_names.add(func_name)
            plugin_name = self.ct.get_plugin_from_function(func_name)
            return f"{self._plugin_namespace(plugin_name)}.{func_name}"
        return func_name
    ...
    def generate_code(self):
            ...
            for selection_type in [
                ...
                for n in nodes:
                    ...
                    if func_meta["class_name"] is None:
                        import_function_names.add(base_name)
                    ...
                    for ip in inputs:
                        ...
                        if ip == "raw":
                            ...
                            read_call = self._qualified_call(
                                read_func, import_function_names
                            )
                    ...
                    if func_meta["class_name"] is not None:
                        ...
                        func_name = f"{self._plugin_namespace(plugin_name)}.{func_name}"
                        ...
                        for op in n["outputs"]:
                            ...
                            if write_func_meta["class_name"] is not None:
                                ...
                                write_call = self._qualified_call(
                                    write_func, import_function_names
                                )
        ...
        # Create header with function bindings resolved from loaded plugins.
        header = self._build_header(import_function_names)
