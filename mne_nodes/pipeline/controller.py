...
class Controller:
    ...
    def convert_to_code(self, node_sequence):
        ...
        data_types = get_datatypes(self.bids_root)
        ...
        for target, nodes in targets.items():
            ...
            if target == "file":
                ...
                for n in nodes:
                    name = n["name"]
                    for data in [i for i in n["inputs"] if i not in loaded_data]:
                        if data == "raw":
                            dt_without_anat = [
                                d for d in data_types if d in self.raw_types
                            ]
                            if len(dt_without_anat) > 1:
                                logging.warning(
                                    f"If connected to raw and multiple-datatypes {dt_without_anat} are available, only the first datatype is chosen for raw (further implementation necessary)."
                                )
                            elif len(dt_without_anat) < 1:
                                logging.warning(
                                    "Connected to raw but raw datatypes found!."
                                )
                                continue
                            data = dt_without_anat[0]
                        # ToDo: Implement loading processing step data
                        if data in self.raw_types:
                            code += f"# Load {data}\n"
                            code += "selected_inputs = ct.get('selected_inputs')\n"
                            code += f"if '{data}' not in selected_inputs:\n"
                            code += self._indent(
                                f"logging.warning('{data} not found in selected inputs.')",
                                1,
                            )
                            code += "else:\n"
                            code += self._indent(f"print('Loading {data}')\n", 1)
                            code += self._indent(
                            ...
                            code += self._indent("bp.update(root=ct.bids_root)", 2)
                            output_data = "raw" if data in self.raw_types else data
                            code += self._indent(
                                f"{output_data} = read_raw_bids(bp)\n", 2
                            )
                            loaded_data.append(output_data)
                    code += self._indent(
                        f"print(f'Executing {name} for {{item}} with the following parameters:')\n",
                        2,
                    )
                    code += self._indent(
                        f"func_params = ct.func_parameters('{name}')", 2
                    )
                    code += self._indent("pprint(func_params)", 2)
