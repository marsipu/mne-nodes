...
class Controller:
    ...
    def convert_to_code(self, instructions):
        ...
        for task, name in instructions:
            if task == "load":
                code += "bp=BIDSPath(,root=ct.bids_root)"
        loaded_data = set()
        modules = {}
        start_name = loaded_data
    ...
    def start(self, node_sequence):
        # Parse instructions
        instructions = {}
        # Load data if first node is not an input-node
        if node_sequence[0][1] != "InputNode":
            func_meta = self.get_function_meta(node_sequence[0][0])
            inputs = func_meta["inputs"]
            for ip in inputs:
                instructions.append(("load", ip))
        for node_name, node_type in node_sequence:
            if node_type == "InputNode":
                for dt in get_datatypes(self.bids_root):
                    instructions.append(("load", dt))
            elif node_type == "FunctionNode":
                instructions.append(("execute", node_name))
        # Generate code file
